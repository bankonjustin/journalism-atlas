...
- **Liz (`liz-editorial`)** — not built, held pending Shadow Lists access (see below). Decisions made 2026-07-07:
  - **Editorial standards doc — found, correcting an earlier wrong claim in this file:** it does NOT live in this repo's `_reference/` (that was a guess based on the `james-design-principles.md` pattern — checked, wrong). It's in `~/Downloads/`, same pattern as `atlas-private-columns.csv`. Four versions exist there: v1.1 (`ATLAS-EDITORIAL-STANDARDS.md`, May), v1.3 (June, incl. a duplicate), and v1.4 (`ATLAS-EDITORIAL-STANDARDS-v1_4.md`, touched July 7 — today). Justin chose **v1.4** as the version to scope `liz-editorial.md` to once it's built.
  - **Global Correspondents cluster output has no canonical, updating file** — only a one-off session note (`Atlas Spidering/sessions/row_bluesky_global_correspondents_clusters_2026-07-02.md`). Tracked as an open item in `Atlas Spidering/sessions/CURRENT_STATE.md` (the live Spidering-side tracker — also correcting an earlier claim here that Atlas Spidering had no `CURRENT_STATE.md`; it has two, and that one is the actively-maintained one).
  - **Shadow Lists access — the actual blocker:** no Google Sheets connector exists in this environment (not installed, not in the connector registry — reconfirmed with a broader keyword search). The only Google connector present (Drive) can read/export a Sheet's contents but has no cell-level write capability. Justin decided to hold `liz-editorial.md` entirely rather than fake write access with a local CSV — needs Justin to check claude.ai/Claude Code connector settings directly (outside a coding session) for a real Sheets integration before this resumes.
